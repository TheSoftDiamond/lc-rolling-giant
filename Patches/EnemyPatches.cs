...
public static class EnemyPatches {
            ...
            try {
                ...
                foreach (var (name, chance) in canSpawnIn) {
                    ...
                    if (CustomConfig.CanSpawnInside && level.Enemies.All(x => x.enemyType != Plugin.EnemyTypeInside)) {
                        level.Enemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeInside, chance));
                    ...
                    if (!CustomConfig.DisableOutsideAtNight && CustomConfig.CanSpawnOutside && level.OutsideEnemies.All(x => x.enemyType != Plugin.EnemyTypeOutside)) {
                        level.OutsideEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutside, CustomConfig.SpawnInOutsideChance));
                    ...
                    if (CustomConfig.DisableOutsideAtNight && CustomConfig.CanSpawnOutside && level.DaytimeEnemies.All(x => x.enemyType != Plugin.EnemyTypeOutsideDaytime)) {
                        level.DaytimeEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutsideDaytime, CustomConfig.SpawnInOutsideChance));
    ...
    private static void HandleSpawnInAny(SelectableLevel[] levels) {
        ...
        foreach (var level in levels) {
            try {
                if (CustomConfig.CanSpawnInside && level.Enemies.All(x => x.enemyType != Plugin.EnemyTypeInside)) {
                    level.Enemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeInside, spawnInAnyChance));
                ...
                if (!CustomConfig.DisableOutsideAtNight && CustomConfig.CanSpawnOutside && level.OutsideEnemies.All(x => x.enemyType != Plugin.EnemyTypeOutside)) {
                    if (spawnInAnyOutsideChance != 0) {
                        level.OutsideEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutside,spawnInAnyOutsideChance));
                ...
                if (CustomConfig.DisableOutsideAtNight && CustomConfig.CanSpawnOutside &&
                    ...
                    if (spawnInAnyOutsideChance != 0) {
                        level.DaytimeEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutsideDaytime,spawnInAnyOutsideChance));
            ...
            try {
                ...
                if (!level.spawnableScrap.Any(x => x.spawnableItem == Plugin.PosterItem)) {
                    ...
                    if (!string.IsNullOrEmpty(scrap.Item1)) {
                        level.spawnableScrap.Add(new SpawnableItemWithRarity(Plugin.PosterItem,scrap.Item2));
    ...
    private static void AddGiantToDebugList(QuickMenuManager __instance) {
        ...
        if (enemies.All(x => x.enemyType != Plugin.EnemyTypeInside)) {
            enemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeInside, firstEnemy.rarity));
        ...
        if (outsideEnemies.All(x => x.enemyType != Plugin.EnemyTypeOutside)) {
            outsideEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutside,firstEnemy.rarity));
        ...
        if (daytimeEnemies.All(x => x.enemyType != Plugin.EnemyTypeOutsideDaytime)) {
            daytimeEnemies.Add(new SpawnableEnemyWithRarity(Plugin.EnemyTypeOutsideDaytime,firstEnemy.rarity));
